...
The syncing part is done, now, there is one problem, how will we send request to that agent, previously, we have created router-server , service, *.preview.localhost and all they could do is give response to previewURL. with the help of previewURL only we could access the vite dev server running on 5173 port. Now, we'll do it for *.agent.localhost so that the agent can also accept the requests.
