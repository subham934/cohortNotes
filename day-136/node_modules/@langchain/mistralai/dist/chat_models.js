...
import { AIMessage, AIMessageChunk, ChatMessageChunk, FunctionMessageChunk, HumanMessage, HumanMessageChunk, ToolMessageChunk } from "@langchain/core/messages";
...
function convertMessagesToMistralMessages(messages) {
	...
	const getTools = (message) => {
		if (AIMessage.isInstance(message) && !!message.tool_calls?.length) return message.tool_calls.map((toolCall) => ({
			...toolCall,
	...
	return messages.flatMap((message) => {
		...
		else if (AIMessage.isInstance(message)) if (toolCalls === void 0) return [{
			role: getRole(message.getType()),
			content
		}];
		else {
	...
	*/
	...
	addAllHooksToHttpClient() {
		try {
			...
			if ([
				...
				this.responseHooks
			].some((hook) => hook && hook.length > 0) && !this.httpClient) this.httpClient = new HTTPClient();
	...
	withStructuredOutput(outputSchema, config) {
		...
		if (method === "jsonMode") {
			...
			const asJsonSchema = isInteropZodSchema(schema) || isSerializableSchema(schema) ? toJsonSchema(schema) : void 0;
			llm = this.withConfig({
			...
			llm = this.bindTools([{
				...
				tool_choice: "any",
				...(isInteropZodSchema(schema) || isSerializableSchema(schema)) && { ls_structured_output_format: {
