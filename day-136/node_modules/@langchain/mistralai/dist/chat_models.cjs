...
let _mistralai_mistralai = require("@mistralai/mistralai");
let _mistralai_mistralai_lib_http_js = require("@mistralai/mistralai/lib/http.js");
...
function convertMessagesToMistralMessages(messages) {
	...
	const getTools = (message) => {
		if (_langchain_core_messages.AIMessage.isInstance(message) && !!message.tool_calls?.length) return message.tool_calls.map((toolCall) => ({
			...toolCall,
			id: require_utils._convertToolCallIdToMistralCompatible(toolCall.id ?? "")
		})).map(_langchain_core_output_parsers_openai_tools.convertLangChainToolCallToOpenAI);
	};
	...
	return messages.flatMap((message) => {
		...
		if ("tool_call_id" in message && typeof message.tool_call_id === "string") return [{
		...
		else if (_langchain_core_messages.AIMessage.isInstance(message)) if (toolCalls === void 0) return [{
			role: getRole(message.getType()),
			content
		}];
		else {
			...
			if (filteredToolCalls.length === 0) {
				if (typeof content === "string" && content.trim() === "" || Array.isArray(content) && content.length === 0) return [];
				return [{
					role: getRole(message.getType()),
					content
				}];
	...
	switch (message.role) {
		case "assistant": {
			...
			for (const rawToolCall of rawToolCalls) try {
				const parsed = (0, _langchain_core_output_parsers_openai_tools.parseToolCall)(rawToolCall, { returnId: true });
				toolCalls.push({
					...parsed,
					id: parsed.id ?? (0, _langchain_core_utils_uuid.v4)().replace(/-/g, "")
				});
			} catch (e) {
		...
		default: return new _langchain_core_messages.HumanMessage({ content });
	}
}
function _convertDeltaToMessageChunk(delta, usage) {
	if (!delta.content && !delta.toolCalls) {
		if (usage) return new _langchain_core_messages.AIMessageChunk({
			content: "",
			usage_metadata: usage ? {
				input_tokens: usage.promptTokens ?? 0,
				output_tokens: usage.completionTokens ?? 0,
	...
	const rawToolCallChunksWithIndex = delta.toolCalls?.length ? delta.toolCalls?.map((toolCall, index) => ({
		...toolCall,
		index,
		id: toolCall.id ?? (0, _langchain_core_utils_uuid.v4)().replace(/-/g, ""),
		type: "function"
	})) : void 0;
	let role = "assistant";
	...
	if (role === "user") return new _langchain_core_messages.HumanMessageChunk({ content });
	else if (role === "assistant") return new _langchain_core_messages.AIMessageChunk({
		content,
		...
		if ((0, _langchain_core_utils_function_calling.isLangChainTool)(tool)) {
			...
			return {
				...
				function: {
					name: tool.name,
					description,
					parameters: (0, _langchain_core_utils_types.isInteropZodSchema)(tool.schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(tool.schema) : tool.schema
				}
			};
		}
...
* <br />
*/
var ChatMistralAI = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
	...
	*/
	...
	constructor(modelOrFields, fieldsArg) {
		...
		super(fields);
		this._addVersion("@langchain/mistralai", "1.1.0");
		const apiKey = fields.apiKey ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("MISTRAL_API_KEY");
	...
	*/
	...
	async completionWithRetry(input, streaming) {
		const caller = new _langchain_core_utils_async_caller.AsyncCaller({ maxRetries: this.maxRetries });
		const client = new _mistralai_mistralai.Mistral({
			apiKey: this.apiKey,
		...
		return caller.call(async () => {
			try {
				let res;
				if (streaming) res = await client.chat.stream({
					...input,
					stream: true
				});
	...
	async _generate(messages, options, runManager) {
		...
		for (const part of response?.choices ?? []) {
			...
			if (part.finishReason) generationInfo.finishReason = part.finishReason;
			if (response?.model) generationInfo.model = response.model;
			if (Object.keys(generationInfo).length > 0) generation.generationInfo = generationInfo;
	...
	async *_streamResponseChunks(messages, options, runManager) {
		...
		for await (const { data } of streamIterable) {
			...
			const message = _convertDeltaToMessageChunk(delta, this.streamUsage || options.streamUsage ? data.usage : null);
			if (message === null) continue;
			let text = delta.content ?? "";
			if (Array.isArray(text)) text = text[0].type === "text" ? text[0].text : "";
			const generationInfo = { ...newTokenIndices };
			if (data?.model) generationInfo.model = data.model;
			if (choice.finishReason) generationInfo.finishReason = choice.finishReason;
	...
	addAllHooksToHttpClient() {
		try {
			...
			if ([
				...
				this.responseHooks
			].some((hook) => hook && hook.length > 0) && !this.httpClient) this.httpClient = new _mistralai_mistralai_lib_http_js.HTTPClient();
	...
	withStructuredOutput(outputSchema, config) {
		...
		if (method === "jsonMode") {
			...
			const asJsonSchema = (0, _langchain_core_utils_types.isInteropZodSchema)(schema) || (0, _langchain_core_utils_standard_schema.isSerializableSchema)(schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(schema) : void 0;
			llm = this.withConfig({
			...
			llm = this.bindTools([{
				...
				tool_choice: "any",
				...((0, _langchain_core_utils_types.isInteropZodSchema)(schema) || (0, _langchain_core_utils_standard_schema.isSerializableSchema)(schema)) && { ls_structured_output_format: {
