...
*/
function isCodestralModel(model) {
	return model.toLowerCase().includes("codestral");
}
/**
* MistralAI completions LLM.
*/
var MistralAI = class extends _langchain_core_language_models_llms.LLM {
	static lc_name() {
		return "MistralAI";
	}
	lc_namespace = [
...
Either provide one via the "apiKey" field in the constructor, or set the "MISTRAL_API_KEY" environment variable.`);
	...
	*/
	async _call(prompt, options) {
		...
		let content = (await this.completionWithRetry(params, options, false))?.choices?.[0].message?.content ?? "";
						...
						for await (const { data } of stream) {
							...
							for (const part of data.choices) {
								...
								if (!choices[part.index]) choices[part.index] = {
									...
									if (choice.message) choice.message.content = (choice.message.content ?? "") + content;
	...
	async completionWithRetry(request, options, stream) {
		...
		const caller = new _langchain_core_utils_async_caller.AsyncCaller({
		...
		return caller.callWithOptions({ signal: options.signal }, async () => {
			try {
				let res;
				if (this.useFim) if (stream) res = await client.fim.stream({
					...request,
					stream: true
				});
					...
					if (stream) res = await client.chat.stream({
						...chatRequest,
						stream: true
					});
					else res = await client.chat.complete(chatRequest);
				}
				return res;
			} catch (e) {
