...
export const googleCallback = async (req, res) => {
  try {
    if (!req.user) {
      return res.redirect(config.NODE_ENV === 'development' ? "http://localhost:5173/login?error=no_user" : "/login?error=no_user");
    }
    ...
    if (!email) {
      return res.redirect(config.NODE_ENV === 'development' ? "http://localhost:5173/login?error=no_email" : "/login?error=no_email");
    }
    ...
    res.redirect(config.NODE_ENV === 'development' ? "http://localhost:5173/login?error=server_error" : "/login?error=server_error");
  }
};
