...
1. FROM: it is the base image for our application.
2. COPY: it is used to copy the codebase from the host machine to the container.
3. WORKDIR: it is used to set the working directory for our application.
4. EXPOSE: it is used to expose the port for our application.
5. RUN: it is used to run the commands inside the container.
6. CMD: it is used to run the commands inside the container.
7. ENTRYPOINT: it is used to run the commands inside the container.
8. ENV: it is used to set the environment variables for our application.
9. LABEL: it is used to set the labels for our application.
...
# copy the entire current directory to the docker image (since dockerfile is inside day-158 directory, so it will copy the entire day-158 directory),
